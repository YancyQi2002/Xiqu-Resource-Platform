...
async function loadPeakingOperaList() {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    const id = result[i].PeakingOperaId
...
<template>
  ...
  >
    <el-row class="flex items-center justify-center">
      ...
      <button
        class="cursor-default rounded-md border-2 border-sky-500 p-2 text-blue-500"
      >
        不含剧照等图片资源
      </button>
    </el-row>
      ...
      <template #default="props">
        ...
        >
          ...
          <el-form-item label="视频解说">
            <span
              v-if="props.row.dramaInterpretation !== '暂无解说！'"
              class="w-full"
            >
              <iframe
                class="mt-2"
